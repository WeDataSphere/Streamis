...
import com.webank.wedatasphere.streamis.jobmanager.manager.transform.entity.{StreamisJarTransformJobContent, StreamisTransformJob}
...
class FlinkInternalConfigTransform extends FlinkConfigTransform {
  ...
  override def transform(streamisTransformJob: StreamisTransformJob, job: LaunchJob): LaunchJob = {
    streamisTransformJob.getStreamisTransformJobContent match {
      case content: StreamisJarTransformJobContent => {
         content.getResources
      }
      case _ =>
    }
    super.transform(streamisTransformJob, job)
  }
